...
module.exports = async function(groupId) {
    ...
    if (queryData[0].catch === true) return ["skip", ""];
    await collection.update({ "_id": queryData[0]._id }, { $set: { "catch": true } }).catch(err => console.log(`Got some error from update data in got you ->${err}`));
